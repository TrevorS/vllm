...
def _nvfp4_convert_attention(layer) -> None:
    """Convert BF16 or FP8 weight to NVFP4 and prepare for CUTLASS FP4."""
    import torch
    ...
    convert_to_nvfp4_linear_kernel_format(NvFp4LinearBackend.VLLM_CUTLASS, layer)
    layer._use_nvfp4_attn = True
    torch.cuda.synchronize()
    logger.info("NVFP4 attention: quantized %s [%s] -> CUTLASS FP4",
...
def _nvfp4_apply_attention(layer, x, bias=None):
    ...
    return apply_nvfp4_linear(NvFp4LinearBackend.VLLM_CUTLASS, layer, x, bias)
# --- End NVFP4 attention ---
