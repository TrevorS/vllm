...
_NVFP4_ATTN_PATTERNS = (
    ...
    # produces poor quality weights that tank acceptance rate (39 tok/s vs 56)
)
...
def _nvfp4_convert_attention(layer) -> None:
    """Convert BF16 or FP8 weight to NVFP4 and prepare for Marlin FP4."""
    import torch
    ...
    # Get BF16 weights — may need dequantization from FP8/Marlin first
    weight_raw = layer.weight.data
    if weight_raw.dtype in (torch.bfloat16, torch.float16, torch.float32):
        # Already BF16/FP16 — use directly
        weight_bf16 = weight_raw
    else:
        # Quantized (FP8 Marlin int32, or raw FP8) — dequantize
    ...
    # Use MARLIN backend — VLLM_CUTLASS fails FakeTensor shape analysis during
    # torch.compile because it quantizes activations at trace time. Marlin uses
    # concrete size_n/size_k ints for output shape, so tracing works correctly.
    ...
    torch.cuda.synchronize()  # Catch async CUDA errors eagerly
...
def _nvfp4_apply_attention(layer, x, bias=None):
    """Forward pass through Marlin FP4 for NVFP4-converted attention layers."""
    from vllm.model_executor.layers.quantization.utils.nvfp4_utils import (
...
class UnquantizedLinearMethod(LinearMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        # Runtime NVFP4 for attention projections
